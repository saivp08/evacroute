...
def plan_transportation(graph: nx.MultiDiGraph, scenario: ScenarioResponse,
    ...
    for incident in medical:
        ...
        rescue_required = 1 if incident.severity == "high" or incident.type == "RESCUE_INCIDENT" else 0
        ...
        for kind in ("ambulance", "rescue_team"):
            ...
            for resource in candidates:
                if kind == "ambulance" and supported >= injuries:
                    break
        ...
        uncovered = max(0, injuries - supported)
        unfilled = rescue_required - rescue_count
        summaries.append(DispatchSummary(
            incident_id=incident.id, injuries=injuries,
            supported_injuries=min(supported, injuries), uncovered_injuries=uncovered,
    ...
    metrics = EmergencyMetrics(
        **evacuation.metrics.model_dump(), active_medical_incidents=sum(i.type == "MEDICAL_INCIDENT" for i in medical),
        active_rescue_incidents=sum(i.type == "RESCUE_INCIDENT" for i in medical),
