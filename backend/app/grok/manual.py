"""Optional real provider check: python -m app.grok.manual (never run by pytest)."""
...
def main():
    ...
    if not os.getenv("GROK_API_KEY", "").strip():
        print("Live Grok check skipped: GROK_API_KEY is not set. No mock parser was used.")
        return 2
    report = json.loads(Path(__file__).with_name("demo_reports.json").read_text(encoding="utf-8"))[0]["report"]
    graph = load_graph()
    try:
        plan = parse_and_apply(report, graph, build_scenario(graph), IncidentState(), GrokClient())
    except (GrokError, TargetNotFound, InfeasiblePlan) as error:
        print(f"Live Grok check failed ({type(error).__name__}). No running server state was changed.")
        return 1
    print("LIVE Grok response, validated and applied to an isolated in-memory scenario:")
