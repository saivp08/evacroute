"""Optional real Anthropic check: python -m app.openai.manual (never run by pytest)."""
...
def main():
    ...
    if not os.getenv("ANTHROPIC_API_KEY", "").strip():
        print("Live Anthropic check skipped: ANTHROPIC_API_KEY is not set. No mock parser was used.")
        return 2
    ...
    try:
        ...
        print(f"Live Anthropic check failed ({type(error).__name__}). No running server state was changed.")
        return 1
    print("LIVE Anthropic response, validated and applied to an isolated in-memory scenario:")
